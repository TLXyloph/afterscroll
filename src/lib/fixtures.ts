export function fixture(path: string, _init?: RequestInit): unknown {
  ...
  if (path === '/api/sync') return { synced: 3, todos: 2, events: 1, insights: 2, needsReview: 0 };
  ...
  if (path === '/api/ask') return { answer: 'You saved a tip about prewarming caches on deploy so traffic spikes never hit a cold cache. [1]', sources: [{ text: 'Prewarm caches on deploy, not on first request. Source: https://x.com/i/status/seed-001' }],  };
