...
export async function POST(_req: Request, { params }: { params: Promise<{ id: string }> }) {
  ...
  try {
    const rows = await sq<any>(`SELECT TITLE, START_TS, DURATION_MIN FROM EVENT_SUGGESTIONS WHERE ID = ?`, [id]);
    ...
    await insertCalendarEvent(e.TITLE, startNaive, Number(e.DURATION_MIN ?? 30));
    await sq(`UPDATE EVENT_SUGGESTIONS SET ADDED = TRUE WHERE ID = ?`, [id]);
